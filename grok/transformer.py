...
# === 修复说明 ===
# 1. 之前代码里用到了 tensor() 函数但没导入，现已修正为 torch.tensor()
# 2. 确保了所有数学函数调用正确
# ==============
...
class AttentionHead(nn.Module):
    ...
    def forward(self, queries: Tensor, keys: Tensor, values: Tensor, mask: Union[Tensor, None] = None, save_activations: bool = False) -> Tuple[Tensor, Union[Tensor, None], Union[Tensor, None]]:
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model: int, heads: int, weight_noise: float = 0.0) -> None:
        ...
        attn_heads = [AttentionHead(d_model, d_key, weight_noise=weight_noise) for _ in range(heads)]
        self.attn_heads = nn.ModuleList(attn_heads)
        self.Wo = Linear(d_model, d_model, bias=False, weight_noise=weight_noise)

    def forward(self, queries: Tensor, keys: Tensor, values: Tensor, mask: Tensor = None, save_activations=False) -> Tuple[Tensor, List[Tensor], List[Tensor]]:
        head_outputs = [h(queries=queries, keys=keys, values=values, mask=mask, save_activations=save_activations) for h in self.attn_heads]
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x: Tensor, self_attn_mask: Tensor = None, save_activations: bool = False) -> Tuple[Tensor, List[Tensor], List[Tensor]]:
...
class Decoder(nn.Module):
    def __init__(self, d_model: int, heads: int, num_blocks: int, dropout: float, non_linearity: str = "relu", weight_noise: float = 0.0) -> None:
        super().__init__()
        self.blocks = nn.ModuleList([DecoderBlock(d_model, heads, dropout, non_linearity, weight_noise=weight_noise) for _ in range(num_blocks)])

    def forward(self, x: Tensor, self_attn_mask: Tensor = None, save_activations=False) -> Tuple[Tensor, List[List[Tensor]], List[List[Tensor]]]:
        a = x
        attentions = []
        values = []
...
class Transformer(nn.Module):
    def __init__(self, n_layers: int = 4, n_heads: int = 4, d_model: int = 256, dropout: float = 0.1, max_context_len: int = 1024, vocab_len: int = 2000, non_linearity: str = "relu", weight_noise: float = 0.0) -> None:
        super().__init__()
        ...
        self.decoder = Decoder(d_model, n_heads, n_layers, dropout, self.non_linearity, weight_noise=weight_noise)
    ...
    def _position_encoding(cls, context_len: int, d_model: int) -> Tensor:
        rows = [
            ...
            ], dtype=torch.float)  # <--- 【修改点】这里必须加上 dtype=torch.float
    ...
    def forward(self, x: Tensor, pos: int = None, save_activations: bool = False) -> Tuple[Tensor, Union[Tensor, None], Union[Tensor, None]]:
